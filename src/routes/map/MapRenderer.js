  ...
  */
  ...
  createEffect(() => {
    ...
    const marker = new maptilersdk.Marker({
      ...
      scale: 1,
      draggable: true
    }).setLngLat(props.longitude && props.latitude ? [props.longitude, props.latitude] : [44.7833, 41.7167]).addTo(map)
    ...
    if (!props.longitude && !props.latitude && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          marker.getPopup().setHTML(`<p class="text-sm font-[thin-font] font-bold text-gray-500">${parsed.placeNameKa}</p>`);
        },
        (error) => {
    ...
    function parseGeocodeResult(geojson) {
      ...
      const result = {
        ...
        region: null,
        quarter: null,
        longitude: geojson.query[0],
        latitude: geojson.query[1]
      };
      ...
      if (Array.isArray(feature.context)) {
        for (const ctx of feature.context) {
          ...
          } else if (ctx.id.startsWith("municipality.")) {
            result.district = ctx.text_ka;
            if (!result.city) result.city = ctx.text_ka
          } else if (ctx.id.startsWith("municipal_district.") || ctx["osm:place_type"] === "town" || ctx["osm:place_type"] === "village") {
            result.city = ctx.text_ka;
          } else if (ctx["osm:tags"]?.place === "village") {
            result.city = ctx.text_ka;
          }
          else if (ctx.id.startsWith("place.") && ctx["osm:place_type"] !== "town" && ctx["osm:place_type"] !== "village") {
    ...
    marker.on('dragend', async (e) => {
      ...
      props.setMarkedLocation(parsed)
      marker.getPopup().setHTML(`<p class="text-xs font-[thin-font] font-bold text-gray-600">${parsed.placeNameKa}</p>`);
    });

    onCleanup(() => map.remove());
  });

  return <div ref={el => mapContainer = el} style={{ height: props.height, width: props.width }} />;
}