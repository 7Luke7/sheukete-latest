...
import { get_all_special_notifications } from "../../api/notifications/specials";

const SpecialNotifications = () => {
  const initialNotifications = createAsync(get_all_special_notifications, {
    deferStream: true,
  });
  ...
  const fetchData = async () => {
    ...
    try {
      const data = await get_all_special_notifications(
  ...
  return (
    <>
      <Suspense fallback={<p>"Loading..."</p>}>
        <h1 class="font-[normal-font] text-xl pl-4">სიახლეები & შეთავაზებები({initialNotifications()?.totalCount})</h1>
