...
const Xelosani = (props) => {
  ...
  return (
    <MetaProvider>
      <Title>{`Sheukete.ge: ${user()?.firstname} ${user()?.lastname}`}</Title>
      <Meta
        name="description"
        content={`Sheukete.ge: ${user()?.firstname} ${user()?.lastname
          } ხელოსანი`}
      ></Meta>
      <Base
        target="_blank"
      ...
      />
      <Switch>
        <Match when={props.location.search.includes("/friends/all")}>
          <Friends>
        ...
        <Match when={true}>
          ...
          <div class="relative">
            <div class="w-[90%] mx-auto relative my-8">
              <Show when={user()}>
                <Show
                  when={user().status === 200 && user().stepPercent !== 100}
                >
                  <div
                    class={`${modal() && "blur-[0.8px] pointer-events-none"
                ...
                <Show when={toast()}>
                  <div
                    class={`${isExiting() ? "toast-exit" : "toast-enter"
